...
CREATE TABLE users (
  ...
  insert_date DATE DEFAULT SYSDATE NOT NULL,
  -- 拡張機能用追加カラム
  purchase_count NUMBER(10) DEFAULT 0,          -- 購入回数
  theme_id NUMBER(1,0) DEFAULT 0,                -- きせかえテーマID
  total_purchase_amount NUMBER(10) DEFAULT 0     -- 累計購入金額
);

-----------------------------------------------------------------------
-- 注文テーブルの作成（クーポン情報・外部キー制約を最初から組み込み）
-----------------------------------------------------------------------
CREATE TABLE orders (
  id NUMBER(6) PRIMARY KEY,
  postal_code VARCHAR2(7) NOT NULL,
...
-----------------------------------------------------------------------
-- レビューテーブルの作成（JavaのEntity仕様に完全に一致）
-----------------------------------------------------------------------
CREATE TABLE reviews (
    id NUMBER(10) PRIMARY KEY,
    item_id NUMBER(10) NOT NULL,
    user_id NUMBER(10) NOT NULL,
    evaluation NUMBER(1) NOT NULL, -- ORA-00904対策：Java側のフィールド名 evaluation に修正
    content VARCHAR2(1000) NOT NULL, -- 元の body カラムから Java側の content フィールド名に修正
    delete_flag NUMBER(1) DEFAULT 0 NOT NULL,
    insert_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
    stamp NUMBER(10) DEFAULT 0, -- 前回の ORA-00904対策用カラム
...
-- レコード登録(会員)
-----------------------------------------------------------------------
INSERT INTO users VALUES(seq_users.NEXTVAL, 'tanaka_taro@test.co.jp', 'Testtest0', 'システム管理太郎', '1111111', '東京都台東区1-2-3 ABCビル10階', '0123456789', 0, DEFAULT, DEFAULT, 0, 0, 0);
INSERT INTO users VALUES(seq_users.NEXTVAL, 'unyo_jiro@test.co.jp', 'Testtest1', '運用管理二郎', '1111111', '東京都台東区1-2-3 ABCビル10階', '0123456789', 1, DEFAULT, DEFAULT, 0, 0, 0);
INSERT INTO users VALUES(seq_users.NEXTVAL, 'ippan_saburo@test.co.jp', 'Testtest2', '一般三郎', '1111111', '東京都台東区4-5-6 ABCマンション5階', '0123456789', 2, DEFAULT, DEFAULT, 0, 0, 0);
