...
public class ClientOrderRegistController {
	...
	public String showOrderComplete(@RequestParam(name = "usePoint", required = false) Integer usePoint,
		...
		// ポイント利用の確認画面をスキップ（要件に基づくスムーズな操作のため）
		...
		if (couponBean != null) {
			...
			discount = priceCalc.calculateDiscount(total, couponBean);
		...
		order.setPaymentAmount(total - discount - usePoint);
		...
		int finalPaymentAmount = total - discount - usePoint;
		int earnPoint = finalPaymentAmount / 10;
		if (earnPoint > 0) {
